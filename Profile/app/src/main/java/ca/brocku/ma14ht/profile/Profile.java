...
public class Profile extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        email.setText("ma14ht@brocku.ca");
    ...
    private boolean checkIfCurrentPasswordMatches() {
        ...
        DatabaseReference users = FirebaseDatabase.getInstance().getReference(e);
        DatabaseReference userPassword = users.child("password");//Get the password stored.
    ...
    private void initializeComponents() {
        ...
        save.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                user.addChildEventListener(cel);
            }
        });
    }
    ...
    private void updateData(DataSnapshot dataSnapshot) {
        ...
        if (userEmail.toLowerCase().equals(email.getText().toString().substring(0, 6).toLowerCase
                ())) {
            user.child(userEmail).child("firstName").setValue(newFirstName);
            user.child(userEmail).child("lastName").setValue(newLastName);
            ...
            if (ifValidInput(newPassword, 8, 32, 2) && checkIfCurrentPasswordMatches()) {
                user.child(userEmail).child("password").setValue(newUpdatedPassword);
    ...
    /**
     * For testing purposes, this method will populate Firebase. It will add 6 entries.
     */
    private void populateDatabase() {
        String[] emails = new String[]{"MA14HT", "KT13NH", "DP11BQ", "MA10ES", "WA11EU", "RM13JP"};
        String[] names = new String[]{"Maysara", "Kevin", "Diwash", "Maher", "Wes", "Ronny"};
        for (int i = 0; i < emails.length; i++) {
            user.child(emails[i]).child("firstName").setValue(names[i]);
            user.child(emails[i]).child("lastName").setValue("last name " + names[i]);
            user.child(emails[i]).child("email").setValue(emails[i]);
            user.child(emails[i]).child("password").setValue("passwordD1");
        }
    }
    ...
    private void updateTextboxes() {
        ...
        DatabaseReference userEmail = user.child("email");//Get the email stored.
        DatabaseReference userFirstName = user.child("firstName");//Get the first name stored.
        DatabaseReference userLastName = user.child("lastName");//Get the last name stored.
